...
# is *used*, not before sglang is imported. Applying them eagerly cost ~2 s per
# process (it imports torch + transformers, and transformers.masking_utils pulls
# in torch._dynamo). Apply now if transformers is already loaded, otherwise the
# moment `import transformers` completes, wherever that happens.
...
# Frontend language API (sglang.lang.*): resolved on first attribute access so
# that server-side processes do not pay for aiohttp/requests/the interpreter.
_LAZY_ATTRS = {
    **{
