...
def _check_cuda_device_version(
    ...
    if not is_cuda():
        return False
    # get_device_sm() answers via NVML before torch.cuda is initialized, so
    # platform probes evaluated at import time do not create a CUDA context.
    return (
...
def get_device_module():
    ...
    # accelerator, which initializes the CUDA runtime (cudaGetDeviceCount) as a
    # side effect. Several modules call this at import time, so every sglang
    # process paid a CUDA runtime init on import and became unusable as a
    # fork() parent. Resolve the device type from the platform checks instead.
    try:
...
def _get_device_sm_via_nvml() -> Optional[int]:
    """Compute capability of the first visible GPU via NVML, without touching
    torch.cuda's lazy init (which creates a context and marks the process as
    unsafe to fork)."""
    try:
        import pynvml
    except ImportError:
...
def get_device_sm():
    if torch.cuda.is_available() or is_musa():
        # get_device_sm() is evaluated at import time (deep_gemm_wrapper
        # configurer). Prefer NVML so that importing sglang does not initialize
        # the CUDA context in every process (launcher, detokenizer, DP
...
def _load_image(
    ...
    if is_jpeg_with_cuda(image_bytes, gpu_image_decode):
        try:
            ...
            # Lazy: torchvision (and torch._dynamo behind it) costs ~1.3 s at import
            # and is only needed for GPU JPEG decode.
