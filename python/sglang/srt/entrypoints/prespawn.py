...
publish, ports), spawns the schedulers (or the DP controller) with lazy entry
points, and parks the result here; ``_launch_subprocesses`` adopts it via
``take()`` and skips the preparation it already did. Combined with
...
class Prespawned(msgspec.Struct):
    ...
    resources: Any  # worker_launch.LaunchResources
...
def maybe_prespawn(server_args: ServerArgs) -> None:
    ...
    try:
        resources = allocate_launch_resources(
            server_args, port_args=None, launch_weight_cache_daemons=_no_daemons
        )
    ...
    _PRESPAWNED = Prespawned(
        server_args=server_args, resources=resources, result=result, procs=procs
    )
...
def _no_daemons(server_args: ServerArgs) -> List[Any]:
    # eligible() excludes weight_cache_mode == "daemon".
    raise AssertionError("prespawn does not launch weight-cache daemons")
