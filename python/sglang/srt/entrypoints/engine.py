...
from sglang.srt.entrypoints.worker_launch import (  # noqa: F401  (re-exported)
    SchedulerInitResult,
...
class Engine(EngineScoreMixin, EngineBase):
    ...
    def _launch_scheduler_processes(
        ...
        *,
        ...
        The implementation lives in `worker_launch` so that `prespawn` can
        start the workers before this module (and its import graph) is loaded.

        Returns:
            Tuple of (SchedulerInitResult, scheduler_procs).
            scheduler_procs is None for RayEngine (uses Ray actors instead).
        """
        return launch_scheduler_processes(
            server_args,
            port_args,
    ...
    def _launch_subprocesses(
        ...
        # Workers started early by prespawn.maybe_prespawn() (right after
        # argument parsing, before this module was imported) are adopted here;
        # it already ran the resolution, publish and allocation below.
        ...
        if pre is not None and run_scheduler_process_func is not run_scheduler_process:
            prespawn.abandon(pre)  # a custom entry point cannot adopt them
            pre = None
        if pre is not None:
            resources = pre.resources
            scheduler_init_result, scheduler_procs = pre.result, pre.procs
        else:
            context_before_publish = resolve_and_publish(server_args)
            ...
            try:
                resources = allocate_launch_resources(
                    server_args,
                    port_args=port_args,
                    launch_weight_cache_daemons=cls._launch_weight_cache_daemons,
                )
            except BaseException:
        ...
        port_args = resources.port_args
        engine_info_bootstrap_server = resources.engine_info_bootstrap_server
        weight_cache_daemon_procs = resources.weight_cache_daemon_procs
