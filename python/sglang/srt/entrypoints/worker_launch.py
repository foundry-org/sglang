"""Worker process launch, kept free of the model / tokenizer / HTTP import graph.

`sglang.srt.entrypoints.engine` re-exports everything here (it used to define
it), so importers of the engine module see no change. Keeping the launch path
in a light module lets `prespawn.maybe_prespawn()` prepare the launch and start
the scheduler / DP-controller processes right after argument parsing, before
the launcher imports its HTTP and tokenizer stack, so the workers' own init
overlaps that import.
"""
...
class LaunchResources(msgspec.Struct):
    """What `allocate_launch_resources` set up before the workers spawn."""

    port_args: Any  # PortArgs
    engine_info_bootstrap_server: Optional[Any] = None
    weight_cache_daemon_procs: List[Any] = []


def resolve_and_publish(server_args: ServerArgs):
    """Everything the launcher does to the config before it spawns workers.

    Returns the context snapshot taken before `publish`; a caller that fails
    after this restores it so the process keeps the context it had.
    """
    ...
    t_parsers = time.perf_counter() - t2

    # This publish replaces whatever was published before it, so a rollback
    # restores that rather than clearing the process: a caller that catches
    # the launch error still has the context it had.
    context_before_publish = snapshot_context()
    publish(server_args, role="tokenizer")
    logger.debug(
        "Launch preparation: resolve %.2fs, validate %.2fs, parser detection %.2fs",
        t_resolve,
        t_check,
        t_parsers,
    )
...
def allocate_launch_resources(
    server_args: ServerArgs,
    *,
    port_args: Optional[PortArgs],
    launch_weight_cache_daemons: Callable[[ServerArgs], List[Any]],
) -> LaunchResources:
    """Ports, the per-rank engine-info bootstrap server and the weight-cache
    daemons (daemon mode only). Runs after `resolve_and_publish`."""
    ...
    if get_model().weight_cache_mode == "daemon":
        weight_cache_daemon_procs = launch_weight_cache_daemons(server_args)
    return LaunchResources(
        port_args=port_args,
        engine_info_bootstrap_server=engine_info_bootstrap_server,
        weight_cache_daemon_procs=weight_cache_daemon_procs,
    )
...
def launch_scheduler_processes(
    ...
    run_dp_controller_func: Callable,
) -> Tuple[SchedulerInitResult, Optional[List]]:
    """Launch scheduler processes (or the DP controller) using multiprocessing.
